...
export async function POST(req: Request) {
  try {
    if (!process.env.GEMINI_API_KEY) {
      return NextResponse.json({ error: "Error: API Key is missing. Please check your configuration." }, { status: 500 });
    }
    ...
    const filteredHistory = history.slice(0, -1).filter((msg: Message, index: number, arr: Message[]) => {
      ...
      if (arr.slice(0, index).every(prevMsg => prevMsg.role === Sender.Bot) && msg.role === Sender.Bot) {
        return false;
      }
      return true;
    });
    ...
    return NextResponse.json({ error: error.message || "I'm having trouble connecting to my knowledge base right now. Please try again later." }, { status: 500 });
  }
}
